package com.empresa.broker.audit;
...
class AuditoriaServiceTest {
    ...
    void registrar_devePersistirAuditoria() {
        ...
        auditoriaService.registrar(OperacaoAuditoria.PROCESSAMENTO_KAFKA, 1L, "detalhe");

        verify(auditoriaRepository).registrar(
                eq("PROCESSAMENTO_KAFKA"), eq("CLIENTE"), eq(1L), eq("corr-1"), eq("detalhe"));
    }
    ...
    void registrarErro_deveUsarOperacaoErro() {
        ...
        verify(auditoriaRepository).registrar(
                eq("ERRO"), eq("CLIENTE"), eq(1L), eq(null), eq("falha"));
    }
}
